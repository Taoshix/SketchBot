    ...
    public class CommandsOld : ModuleBase<SocketCommandContext>
    {
            ...
            if (!Context.User.IsBot)
            {
                ...
                if (type == "tokens")
                {
                    if (index >= 1)
                    {
                        ...
                        var list = Database.GetAllUserStats(user as IGuildUser);
                ...
                {
                    await ReplyAsync("Usage: " + ServerSettingsDB.GetSettings(Context.Guild.Id).FirstOrDefault().Prefix + "leaderboard <type> <page>" +
                        "\nAvailable types:" +
                        "\nTokens, Leveling");
                }
            }
        ...
        public async Task Daily([Remainder] IGuildUser user = null)
        {
            ...
            if ((tableName?.Daily.ToString() == "0001-01-01 00:00:00") || (daily.DayOfYear < now.DayOfYear && difference < 0 || difference >= 0 || daily.Year < now.Year))
            {
                ...
                {
                    await ReplyAsync($"You would have gotten 4x more tokens if you have voted today. See {ServerSettingsDB.GetSettings(Context.Guild.Id).FirstOrDefault()?.Prefix}upvote");
                }
